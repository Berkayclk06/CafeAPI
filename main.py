...
def update_price(cafe_id):
    ...
    if cafe:
        ...
        return jsonify(error={"No Found": "Sorry a cafe with that id was not found in the database."})
...
def delete_cafe(cafe_id):
    ...
    if api_key == API_KEY:
        ...
        if cafe:
            ...
            db.session.commit()
            return jsonify(response={"success": "Successfully deleted the coffee."}), 200
        else:
            return jsonify(error={"No Found": "Sorry a cafe with that id was not found in the database."}), 404
    else:
